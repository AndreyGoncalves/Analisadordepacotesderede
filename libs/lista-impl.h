...
template <typename T> T& lista<T>::obtem(int posicao) {
...
template <typename T> T& lista<T>::operator[](int pos) {
    return obtem(pos);
}
...
template <typename T> T& lista<T>::procura(const T& algo) const {
    typename std::list<T>::iterator it = std::find(this->begin(), this->end(), algo);
...
template <typename T> lista<T>& lista<T>::sublista(unsigned int pos1, unsigned int pos2, lista<T> & outra) const {   
    ...
    typename lista<T>::iterator it1, it2;
    pos2 -= pos1;
    for (;pos1 > 0; it1++, pos1--);
